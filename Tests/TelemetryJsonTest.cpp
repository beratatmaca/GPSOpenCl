...
{
    GPSOpenCl::TelemetryExporter::TelemetrySnapshot snapshot{};
    ...
    GPSOpenCl::TelemetryExporter::SatelliteTelemetry sat{};
    sat.prn = 7;
    sat.cn0 = std::numeric_limits<float>::quiet_NaN();
    snapshot.satellites.push_back(sat);

    const std::string jsonText = GPSOpenCl::TelemetryExporter::formatTelemetryJson(snapshot);
